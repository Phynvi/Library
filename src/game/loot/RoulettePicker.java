...
public class RoulettePicker<L extends LootableItem> {
	...
	public L next() {
		if (loot.isEmpty())
			throw new IllegalStateException("No loot available to select: table empty");

		double value = random.nextDouble() * this.chanceValueTotal;

		for (L lootable : this.loot) {
			value -= lootable.getChance();
			if (value <= 0)
				return lootable;
		}
		return null;
