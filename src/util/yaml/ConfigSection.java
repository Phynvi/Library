...
public class ConfigSection implements Map<String, Object> {
    ...
    /**
     * Constructs a new {@code ConfigSection} with values based on the YAML
     * parsed from the specified {@code string}.
     * 
     * @param string
    ...
    /**
     * Constructs a new {@code ConfigSection} with values based on the YAML
     * parsed from the specified {@code InputStream}.
     * 
     * @param in
    ...
    /**
     * Sets the object at the given location to the given object. If you attempt
     * to do this with an object which is not serializable, then chances are
     * you'll end up with garbage data. Consider writing a method to serialize
     * the required object into a ConfigSection if this is the case. Numbers,
     * Strings and ConfigSections are stored properly and may be fetched later.
     * 
     * @param s
    ...
    /**
     * Fetches the map at the specified key, or if it is null or not a map,
     ...
     * @param v
     *            the maps value class, eg. Integer.class (For a Map<?,
     *            Integer>)
     * @param fallback
    ...
    /**
     * Fetches the byte array at the specified key, or returns the given
     * fallback if it was not found or invalid.
     * 
     * @param key
     *            The key to retrieve
     * @param fallback
    ...
    /**
     * Fetches the byte array at the specified key, or returns null if it was
     ...
     * calling getObject(key). If the object is a collection, its elements are
     * added to a new list. If the object is a map, its keys are added to a new
     * list. This new list is returned. If it's neither a set nor map, then the
     ...
     * calling getObject(key). If the object is a collection, its elements are
     * added to a new list. If the object is a map, its keys are added to a new
     * list. This new list is returned. If it's neither a set nor map, then null
    ...
    /**
     * Fetches a set of strings which represent the keys for this config
     * section. This set is immutable.
     * 
     ...
     *            The key
     * @param fallback
     *            return value if the value was not specified or is not a
     *            ConfigSection
     * @return the section
     */
    ...
    /**
     * Fetches the section under the given key, or returns a blank config
     * section if there was an issue. Note that this differs from other methods
     * which return a null fallback object. If you desire null on error, use
     * getSection(key, null).
     * 
     * @param key
     *            the key
    ...
    /**
     * Fetches the long at the given key, allowing a fallback value to be
     * specified.
     * 
     * @param k
     *            the key to search for
