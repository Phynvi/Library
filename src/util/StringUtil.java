...
public class StringUtil {
	...
	public static String decompressHuffman(DecodedPacket packet, int numChars) {
		byte[] textBuffer = new byte[packet.getBytes().readableBytes()];
	...
	public static String[] wrap(int lineWidth, boolean wrapWords, String... strings) {
		if (wrapWords) {
			...
			for (int i = 0, index = 0; i < compact.length(); i += lineWidth) lines[index++] = compact.substring(i, Math.min(compact.length(), i + lineWidth));
			return lines;
		}
	}
