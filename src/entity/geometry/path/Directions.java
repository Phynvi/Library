...
public class Directions {
	 ...
	 *           the direction
	 * @param antiClockwise
	 *           true if we're going anticlockwise (Eg NORTH returns NORTH_WEST, SOUTH_EAST returns
	 *           EAST), false if we go clockwise
	...
	/**
	 * Finds the opposite direction of the given direction. Eg, rotate 180 degrees
	 * 
	 * @param d
	 *           the direction
	 ...
	 * @param degrees
	 *           the degrees to rotate it, this must be a multiple of 45. Rotating 360 does nothing.
	 * @return the rotated direction (EG, rotate NORTH 45 degrees equals NORTH_EAST)
	 */
	public static Direction rotate(Direction d, int degrees) {
		int mod = degrees % ALL.length;
		if (mod != 0)
			throw new IllegalArgumentException("Directions must be rotated by a multiple of " + (360 / ALL.length) + " degrees");
		for (int i = 0; i < ALL.length; i++) {
			if (ALL[i] == d) {
	...
	/**
	 * Finds a direction that will have the given directional effects when applied.
	 * 
	 * @param dx
	 *           the north movement intended (Negative becomes south)
	 * @param dy
	 *           the east movement intended (Negative becomes west)
	 * @return the direction
	 * @throws IllegalArgumentException
	 *            if the given values are both 0 or have an absolute value greater than 1.
	 */
