...
public class CacheStore {
    ...
    /**
     * Constructs a new {@code CacheStore} around the index correlating to the
     * specified {@code index} within the {@code cache}.
     * 
     * @param cache
     ...
     * @throws IOException
     *             if there is a problem reading the data or index file within
     *             the path
     */
    ...
    /**
     * Returns an {@code IoBuffer} object with the information contained in the
     * archive with the specified {@code id}. If it does not exist, then an
     * {@code IoBuffer} with an initial capacity of 0 is returned instead.
     * 
     * @param idx
     *            the id of the archive to retrieve the data from
     * @return an {@code IoBuffer} with the archive data; returns an empty
     *         {@code IoBuffer} is non existent
     * @throws IOException
     */
	...
	indexFile.seek(INDEX_IDENTIFIER * id);

	int size = indexFile.readUnsignedMedium();
	int block = indexFile.readUnsignedMedium();
